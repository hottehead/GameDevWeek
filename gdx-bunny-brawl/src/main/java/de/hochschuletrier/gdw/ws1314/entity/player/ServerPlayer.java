...
/**
 * 
 * @author ElFapo ASK BEFORE MODIFYING, OR I'LL MOST CERTAINLY TAKE A SHIT ON
 *         YOUR HEAD! -I'D REALLY LIKE TO SEE THIS xD
 */
...
public class ServerPlayer extends ServerEntity implements IStateListener {
		...
		if (this.isDead) {
			...
			for (Long id : pickedUpEggs) {
				...
				if (entity != null)
					...
					logger.warn("Versucht Ei zurück zu setzten das nicht exsistiert");
			}
			pickedUpEggs.clear();
			currentEggCount = 0;
			return;
		} else if (isInDeadZoneLowerLeft && isInDeadZoneLowerRight
				&& isInDeadZoneUpperLeft && isInDeadZoneUpperRight) {
			if (!this.isOnBridge) {
				this.isDead = true;
	...
	protected void moveBegin(FacingDirection dir) {
		...
		// physicsBody.applyImpulse(dir.getDirectionVector().x *
		// playerKit.getMaxVelocity(),
	...
	public void dropEgg() {
		if (currentEggCount > 0) {
			...
			ServerEgg egg = (ServerEgg) ServerEntityManager.getInstance().getEntityById(
					droppedEggID);
			if (egg != null) {
		...
		if (fixture == fixtureLowerBody) {
			...
			case Ei:
				...
				if (egg.getVisibility()
						&& this.currentEggCount < this.playerKit.getMaxEggCount()
						&& egg.getID() != droppedEggID) {
			...
			case ContactMine:
				break;
			case Carrot:
				applySpeedBuff(ServerCarrot.CARROT_SPEEDBUFF_FACTOR
						- EGG_CARRY_SPEED_PENALTY * currentEggCount,
						ServerCarrot.CARROT_SPEEDBUFF_DURATION);
				ServerEntityManager.getInstance().removeEntity(otherEntity);
				NetworkManager.getInstance().sendEntityEvent(getID(),
						EventType.EAT_PICKUP);
				NetworkManager.getInstance().sendEntityEvent(getID(),
						EventType.BUFF_ACTIVATE_CARROT);

				break;
			case Spinach:
				applyAttackBuff(ServerSpinach.SPINACH_ATTACKBUFF_FACTOR,
						ServerSpinach.SPINACH_ATTACKBUFF_DURATION);
				ServerEntityManager.getInstance().removeEntity(otherEntity);
				NetworkManager.getInstance().sendEntityEvent(getID(),
						EventType.EAT_PICKUP);
				NetworkManager.getInstance().sendEntityEvent(getID(),
						EventType.BUFF_ACTIVATE_SPINACH);
			...
			case SwordAttack:
				...
				if (attack.getSourceID() != getID()
						&& struckBySwordIDs.contains(attack.getSourceID())) {
	...
	public void initPhysics(PhysixManager manager) {
		PhysixBody body = new PhysixBodyDef(BodyType.DynamicBody, manager)
				...
				.fixedRotation(false).gravityScale(0.0f).create();

		// lower body
		body.createFixture(new PhysixFixtureDef(manager).density(DENSITY)
		...
		body.createFixture(new PhysixFixtureDef(manager)
				...
				.shapeBox(WIDTH, HEIGHT * 2.0f - HEIGHT / 2.0f + HEIGHT / 4.0f,
						new Vector2(0.0f, 0.0f), 0.0f).sensor(true));

		// coll check upper left
		body.createFixture(new PhysixFixtureDef(manager).density(DENSITY)
				.friction(FRICTION).restitution(RESTITUTION)
				.shapeBox(1, 1, new Vector2(-HEIGHT / 4, HEIGHT / 4), 0).sensor(true));
	...
	protected void applyKnockback(FacingDirection direction, float impulse) {
		...
		physicsBody.setLinearDamping(BRAKING);
		physicsBody.applyImpulse(direction.getDirectionVector().x * impulse,
				direction.getDirectionVector().y * impulse);
	...
	private void deactivateSpeedBuff() {
		if (speedBuffActive) {
			...
			speedBuffActive = false;
			NetworkManager.getInstance().sendEntityEvent(getID(),
					EventType.BUFF_DEACTIVATE_CARROT);
		}
	}
	...
	private void deactivateAttackBuff() {
		if (attackBuffActive) {
			...
			attackBuffActive = false;
			NetworkManager.getInstance().sendEntityEvent(getID(),
					EventType.BUFF_DEACTIVATE_SPINACH);
